...
crate::impl_into_plot!(impl<T, X, Y> AreaChart<T, X, Y> where X: Clone + PartialEq + Into<SharedString> + 'static, Y: PlotValue);
    ...
    Y: PlotValue,
{
    ...
    fn paint(&mut self, bounds: Bounds<Pixels>, window: &mut Window, cx: &mut App) {
        ...
        // Draw area
        // Unstyled series take the theme's series colors in turn, over a quiet fill.
        ...
        let areas = self.y.iter().enumerate().map(|(i, y_fn)| {
            ...
            let stroke = self
                ...
                .get(i)
                .copied()
                .unwrap_or(chart[i % chart.len()]);
            let fill = self
                .fills
                .get(i)
                .copied()
                .unwrap_or_else(|| stroke.opacity(0.2).into());
            let curve = *self
                .curves
                .get(i)
    ...
    fn tooltip(
        ...
        let chart = cx.omarchy().chart;
        let dot_stroke = cx.omarchy().background;
        let color = |i: usize| {
            self.strokes
                .get(i)
                .copied()
