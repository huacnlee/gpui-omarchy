    ...
    V: PlotValue,
{
    ...
    ///
    /// Default is false: a solid divider hairline, like the shell's separators.
    ...
    fn bar_color(&self, d: &T, frame: Bounds<f32>, bounds: Bounds<Pixels>, cx: &App) -> Hsla {
        let default = cx.omarchy().chart[0];
...
crate::impl_into_plot!(impl<T, B, V> BarChart<T, B, V> where B: Eq + Hash + Into<SharedString> + 'static, V: PlotValue);
    ...
    V: PlotValue,
{
    ...
    fn paint(&mut self, bounds: Bounds<Pixels>, window: &mut Window, cx: &mut App) {
        ...
        let mut axis = PlotAxis::new().stroke(cx.omarchy().border);
        if self.label_axis {
            match alignment {
        ...
        if self.grid {
            let grid = Grid::new().stroke(cx.omarchy().divider());
        ...
        if self.value_axis {
            ...
            let color = cx.omarchy().secondary;
            let texts = self
                .value_tick_labels()
                .into_iter()
        ...
        let default_fill: Background = cx.omarchy().chart[0].into();
        let fill = self.fill.clone();
        let fill_gradient = self.fill_gradient.clone();
        let label_color = cx.omarchy().foreground;
...
mod tests {
    ...
    #[gpui_kit::test]
    fn the_tooltip_swatch_follows_the_bar_color(cx: &mut gpui_kit::TestAppContext) {
        ...
        let bounds = Bounds::new(point(px(0.), px(0.)), gpui_kit::size(px(100.), px(100.)));
        let (default, solid, gradient, stops) = cx.update(|cx| {
            let gain = gpui_kit::green();
            let loss = gpui_kit::red();
            ...
            let gradient = bars()
                .fill(move |_: &f64, _, _, _| {
                    linear_gradient(
                        0.,
                        gpui_kit::linear_color_stop(gain, 0.),
                        gpui_kit::linear_color_stop(loss, 1.),
                    )
                })
        ...
        let accent = cx.update(|cx| cx.omarchy().chart[0]);

        assert_eq!(default, accent);
        assert_eq!(solid, gpui_kit::red());
        assert_eq!(gradient, accent);
        assert_eq!(stops, gpui_kit::green());
    }
