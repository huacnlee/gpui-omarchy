...
crate::impl_into_plot!(impl<T, X, Y> LineChart<T, X, Y> where X: PartialEq + Into<SharedString> + 'static, Y: PlotValue);
    ...
    Y: PlotValue,
{
    ...
    fn paint(&mut self, bounds: Bounds<Pixels>, window: &mut Window, cx: &mut App) {
        ...
        // Draw line
        let stroke = self.stroke.unwrap_or(cx.omarchy().chart[0]);
        let x_fn = x_fn.clone();
        let y_fn = y_fn.clone();
