...
pub use crate::plot::shape::{BarAlignment, SankeyAlign, SankeyLink, SankeyValueScale};
...
/// The spring a hovered pie slice lifts and settles with: Omarchy's popup
/// opacity duration (`PopupCard.qml`).
pub(crate) fn slice_spring() -> gpui_kit::base::Spring {
    gpui_kit::base::Spring::new(std::time::Duration::from_millis(140))
}
...
/// `id`. GPUI takes this same trade-off for [`gpui_kit::Window::use_state`].
#[track_caller]
...
pub(crate) fn pinned_plot_mask(bounds: Bounds<Pixels>, height: f32) -> ContentMask<Pixels> {
    ...
    ContentMask {
        bounds: Bounds::from_corners(
            bounds.origin - gpui_kit::point(bleed, bleed),
            gpui_kit::point(bounds.right() + bleed, bounds.top() + px(height) + bleed),
        ),
    }
}
...
mod tests {
    ...
    use gpui_kit::{Hsla, SharedString, TestAppContext, blue, div, green, point, px, red, size};
    ...
    fn a_named_id_replaces_the_default() {
        assert_eq!(
            ...
            Some(gpui_kit::ElementId::Name("pie".into()))
        );
    }
