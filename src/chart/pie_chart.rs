...
impl<T> PieChart<T> {
    ...
    /// Set the leader line color per slice (defaults to `cx.omarchy().border`).
    pub fn label_line_color(mut self, color: impl Fn(&T) -> Hsla + 'static) -> Self {
        self.label_line_color = Some(Rc::new(color));
        self
    }

    /// Set the label text color (defaults to `cx.omarchy().foreground`).
    ...
    /// The fill of a slice: the per-datum color, or the theme's series colors in turn.
    fn slice_color(&self, index: usize, datum: &T, cx: &App) -> Hsla {
        match self.color.as_ref() {
            Some(color_fn) => color_fn(datum),
            None => {
                let chart = &cx.omarchy().chart;
...
crate::impl_into_plot!(impl<T> PieChart<T>);
...
impl<T> Plot for PieChart<T> {
    fn paint(&mut self, bounds: Bounds<Pixels>, window: &mut Window, cx: &mut App) {
        ...
        for (ix, a) in arcs.iter().enumerate() {
            ...
            let color = self.slice_color(a.index, a.data, cx).opacity(opacity);
            match caches.as_ref() {
        ...
        let label_color = self.label_color.unwrap_or(cx.omarchy().foreground);
        let default_line_color = cx.omarchy().border;
    ...
    fn tooltip(
        ...
        Some(
            ...
            Tooltip::new(cursor, bounds.size)
                .gap(px(8.))
                .row(
                    self.slice_color(state.index, d, cx),
                    name,
