    ...
    Y: Clone + Copy + PartialOrd + Num + ToPrimitive + Sealed + 'static,
{
    ...
    /// Set the text label color (defaults to `cx.omarchy().secondary`).
    ///
...
crate::impl_into_plot!(impl<T, Y> RadarChart<T, Y> where Y: Clone + Copy + PartialOrd + Num + ToPrimitive + Sealed + 'static);
    ...
    Y: Clone + Copy + PartialOrd + Num + ToPrimitive + Sealed + 'static,
{
    ...
    fn paint(&mut self, bounds: Bounds<Pixels>, window: &mut Window, cx: &mut App) {
        ...
        let label_color = self.label_color.unwrap_or(cx.omarchy().secondary);
        let labels = self
            .label_texts
            .iter()
            .enumerate()
...
mod tests {
    ...
    fn test_radar_chart_builder() {
        ...
        assert!(chart.dot);
        assert_eq!(chart.id, gpui_kit::ElementId::Name("radar".into()));
    ...
    fn test_radar_chart_hovered_index() {
        ...
        let bounds =
            gpui_kit::Bounds::new(point(px(0.), px(0.)), gpui_kit::size(px(200.), px(200.)));
