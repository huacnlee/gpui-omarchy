            ...
            .map(|(datum, value)| {
                if let Some(labels) = &self.labels {
                    ...
                    if let Some(node_label) = &self.node_label {
                        lines.push(
                            SankeyLabel::new(node_label(datum)).color(cx.omarchy().secondary),
                        );
                    }
                    lines
                }
...
crate::impl_into_plot!(impl<T> SankeyChart<T>);
            ...
            for line in lines {
                ...
                texts.push(
                    Text::new(
                        text,
                        point(px(x), px(y)),
                        line.color.unwrap_or(cx.omarchy().foreground),
                    )
                    .font_size(font_size)
                    .align(align),
                );
...
mod tests {
    ...
    fn test_sankey_label_builder() {
        ...
        let label = SankeyLabel::new("b").color(gpui_kit::red()).font_size(14.);
        assert_eq!(label.color, Some(gpui_kit::red()));
