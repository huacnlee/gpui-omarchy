//! The hover overlay an Omarchy chart returns from [`Plot::tooltip`](super::Plot::tooltip):
//! a crosshair or band, square data markers, and a compact square box in the
//! shell tooltip's surface.
use gpui_kit::base::motion::spring;
pub use gpui_kit::base::plot::{PlotHover, TooltipState};
use gpui_kit::base::plot::{hover_progress, is_hover_entering, pointer_spring};
use gpui_kit::{
    AnyElement, App, Div, ElementId, FontWeight, Hsla, IntoElement, ParentElement, Pixels, Point,
...
impl CrossLine {
    ...
    fn line(&self, vertical: bool, cx: &App) -> Div {
        let t = cx.omarchy();
        // A dashed crosshair takes the control edge; a band takes the hover fill.
...
#[derive(IntoElement)]
/// A square marker on the hovered datum, with an optional translucent square
/// ring behind it.
pub struct Dot {
    point: Point<Pixels>,
    size: Pixels,
...
impl Dot {
    pub fn new(point: Point<Pixels>) -> Self {
        Self {
            point,
            size: px(6.),
            stroke: gpui_kit::transparent_black(),
...
impl RenderOnce for Tooltip {
    fn render(self, window: &mut Window, cx: &mut App) -> impl IntoElement {
                    ...
                    h_flex()
                        ...
                        .child(
                            h_flex()
                                ...
                                .child(div().text_color(t.secondary).child(row.label)),
                        )
                        // Figures align in the fixed-width face.
                        .child(
                            div()
                                .font_family(t.mono_font.clone())
                                .text_color(row.value_color.unwrap_or(t.foreground))
                                .child(row.value),
                        )
                }))
        } else {
        ...
        let content = content.text_size(rems(0.6875));
        ...
        div()
            ...
            .child(deferred(content.map(|mut this| {
                ...
                this.absolute()
                    .opacity(progress)
                    .when(min_w_unset, |c| c.min_w(rems(9.)))
                    .max_w(rems(20.))
                    .px(rems(0.625))
                    .py(rems(0.375))
                    .border_1()
...
mod tests {
    ...
    fn a_value_color_colors_only_the_row_added_last() {
        let tooltip = Tooltip::new(point(px(0.), px(0.)), gpui_kit::size(px(100.), px(100.)))
            .value_color(gpui_kit::red())
            .row(gpui_kit::blue(), "Open", "1")
            .row(gpui_kit::blue(), "Close", "2")
            .value_color(gpui_kit::green());
        let colors: Vec<_> = tooltip.rows.iter().map(|row| row.value_color).collect();
        assert_eq!(colors, vec![None, Some(gpui_kit::green())]);
    }

    #[test]
    fn a_plain_row_has_no_swatch_and_takes_a_value_color() {
        let tooltip = Tooltip::new(point(px(0.), px(0.)), gpui_kit::size(px(100.), px(100.)))
            .row(gpui_kit::blue(), "Call", "1")
            .plain_row("Total", "3")
            .value_color(gpui_kit::red());
        ...
        assert_eq!(
            rows,
            vec![
                (Some(gpui_kit::blue()), None),
                (None, Some(gpui_kit::red()))
            ]
        );
    }
